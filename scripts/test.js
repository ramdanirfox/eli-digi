const IBX = require('imgbox-js');
const IB = require('imgbox');
// console.log(IBX);
// IBX.imgbox('https://picsum.photos/200/300')
// const b64 = 'data:image/png;base64,';
const url = 'https://picsum.photos/200/300';
// IBX.imgbox(b64)
//     .then(res => console.log('INI HASILNYA', res));
console.log(IB.imgbox);

IB.imgbox(url)
    .then(res => console.log('INI HASILNYABB', res));